...
async function selectGarden(g) {
  ...
  if (userCoords) {
    ...
    if (route) {
      ...
      if (widget && content) {
        if (title) title.textContent = `Driving Directions to ${props.DES_TITLE}`; 
...
//--- 5. UI CONTROL LISTENERS ---//

// 5.1. Set Location via Button
...
// 5.2. Set Location via Enter Key
...
// 5.3 Widget Close logic
document.getElementById("close-widget").onclick = () => {
  document.getElementById("directions-widget").style.display = "none";
};

// 5.4 Directory Panel toggle
...
//--- 7. MOUSE HOVER ---//
map.on('mouseenter', 'Garden_Centroids_Scotland_2-8jwfy7', () => {
  map.getCanvas().classList.add('garden-hover');
});

map.on('mouseleave', 'Garden_Centroids_Scotland_2-8jwfy7', () => {
  map.getCanvas().classList.remove('garden-hover');
});